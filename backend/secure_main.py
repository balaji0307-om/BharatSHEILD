...
def local_qr_fallback(content: str) -> dict:
    ...
    if parsed.scheme == "upi":
        score += 18
        signals.append("Valid UPI payment intent detected")
    ...
    return {
        ...
        "explanation": "QR payload reviewed locally.",
        "what_we_found": f"QR payload decoded for recipient {upi_id}, merchant {merchant}, amount {amount}, note {note}.",
        "why_dangerous": "Recipient ownership cannot be proven from a QR payload alone. Verify the payee inside your UPI app before paying.",
        "how_sure": "70% confidence based on QR payload checks; recipient ownership is not independently verified.",
        "reason_breakdown": [
        ...
        "recommendations": [
            "Verify recipient name, UPI ID, amount, and purpose inside your UPI app.",
        ...
        "qr_analysis": {
            ...
            "recipient_reputation": "Unknown — not independently verified",
            "previous_reports": 0,
            "fingerprint": "BS-QR-LOCAL",
            "risk_signals": signals,
            "checks": [{"label": "Status", "result": "Local QR verification completed"}],
        },
        "local_review": True,
        "mode": "qr",
    }
...
async def analyze_gateway(request: Request):
    payload = await request.json()
    # QR analysis is deliberately deterministic at the gateway. The legacy analyzer
    # can return an `Unable to verify` placeholder even after decoding the QR; that
    # placeholder must never overwrite the usable local QR payload analysis.
