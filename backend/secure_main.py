...
async def security_boundary(request: Request, call_next):
    ...
    if origin and origin not in ALLOWED_ORIGINS: return JSONResponse({"detail": "Origin is not allowed."}, status_code=403)
    length = request.headers.get("content-length")
    if length:
        try:
            if int(length) > MAX_BODY_BYTES: return JSONResponse({"detail": "Request body is too large."}, status_code=413)
    ...
    if request.method != "OPTIONS" and request.url.path.startswith("/api/") and not is_public(request.url.path):
        if not has_valid_session(request): return JSONResponse({"detail": "Authentication required."}, status_code=401)
    if request.url.path == "/api/url-check" and request.method == "POST":
        try:
            payload = await request.json()
            raw = str(payload.get("url", ""))
            parsed = urlparse(raw if "://" in raw else "https://" + raw)
            if parsed.username or parsed.password: return JSONResponse({"detail": "URLs containing embedded credentials are not accepted."}, status_code=400)
...
async def analyze_gateway(request: Request):
    ...
    try:
        ...
        if payload.get("channel") != "qr" or result.get("qr_analysis"): return result
    except Exception: pass
